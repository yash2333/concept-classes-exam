...
function initialData(){
  return {
    nextUserId:2,nextTestId:1,nextQuestionId:1,nextAttemptId:1,nextAnswerId:1,nextEventId:1,
...
app.post("/api/attempts",auth("student"),(req,res)=>{
  ...
  const a={id:db.nextAttemptId++,test_id:t.id,student_id:req.session.user.id,
    score:0,total,violations:0,status:"in_progress",started_at:new Date().toISOString(),submitted_at:null};
...
app.post("/api/attempts/:id/submit",auth("student"),(req,res)=>{
  ...
  a.score=score;a.status="submitted";a.submitted_at=new Date().toISOString();save();
